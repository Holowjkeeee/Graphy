...
{
	public:
		FruchtermanReingold(const adj_list_t& aAdjacencyList, double aK = 15.0);

		void Start(bool aStartCircle = true);
		void Step(int aStepSize, int aMaxStep, std::vector<Vertex>& aPositions);
	...
	private:
		const adj_list_t& m_AdjacencyList;
		double m_K;
		double m_KSquared;
		double m_Temp;
		std::vector<ImVec2> m_Movements;

		bool m_StartCircle = true;
		int m_CurrIter = 0;
		int m_MaxIter = 0;
